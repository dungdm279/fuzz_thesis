 ...
 */
 ...
 void addPath(uint32_t value){
   if(path_loc >= path_len)
   {
     clearPath();
     return;
   }
 
   for(int i=0;i<path_loc;i++)
   {
     if(path[i] == value)
       return;
   }
 ...
 void computeAndUpdateHashInternal(uint32_t &hash, uint32_t value)
 {
     ...
     const int bbhash_bit_count = 19;
     hash = (hash << move_bit) | (hash >> (bbhash_bit_count - move_bit));
     hash &= (1<<bbhash_bit_count) - 1;
     hash ^= value & 0xFF;
     hash = (hash << move_bit) | (hash >> (bbhash_bit_count - move_bit));
     hash &= (1<<bbhash_bit_count) - 1;
     hash ^= (value>>8) & 0xFF;
 ...
 #ifdef DIE_INSTR
 #define SHARED_MEM_PREFIX "DIE_"
 #include <sys/shm.h>
 #else
 #define SHARED_MEM_PREFIX ""
 #endif
 ...
 extern "C" void __sanitizer_cov_trace_pc_guard_init(uint32_t *start, uint32_t *stop) {
     ...
     const char* shm_key = getenv(SHARED_MEM_PREFIX "SHM_ID");
     const char* key_list[bitmap_num];
     for(int i=0;i<bitmap_num;i++)
     {
         char str[20];
         sprintf(str, SHARED_MEM_PREFIX "SHM_ID0%d",i);
         key_list[i] = getenv(str);
     }
     if (!shm_key) {
         puts("[COV] no shared memory bitmap available, skipping");
         __shmem = (struct shmem_data*) malloc(SHM_SIZE);
         for(int i=0;i<bitmap_num;i++)
             __shmem_list[i] = (struct shmem_data*) malloc(MY_SIZE);
     } else {
 #ifdef DIE_INSTR
         ...
         for(int i=0;i<bitmap_num;i++)
             __shmem_list[i] = (struct shmem_data*) shmat(atoi(key_list[i]), 0, 0);
         ...
         for(int i=0;i<bitmap_num;i++){
           if (__shmem_list[i] == MAP_FAILED) {
             fprintf(stderr, "Failed to mmap shared memory %d region\n", i);
             _exit(-1);
           }
         }
 #else
         ...
         for(int i=0;i<bitmap_num;i++)
             fd_list[i] = shm_open(key_list[i], O_RDWR, S_IREAD | S_IWRITE);
         if (fd <= -1) {
         ...
         for(int i=0;i<bitmap_num;i++)
             __shmem_list[i] = (struct shmem_data*) mmap(0, MY_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd_list[i], 0);
         if (__shmem == MAP_FAILED) {
             fprintf(stderr, "Failed to mmap shared memory region\n");
             _exit(-1);
         }
     ...
     {
         char temp[1024];
         sprintf(temp,"/home/piers/fuzz/data/path/%s.txt", file_list[k]);
         fp[k]=fopen(temp, "a");
     }  
     be_able_to_trace_pc_guard = 1;
 }
 
 extern "C" void __sanitizer_cov_trace_pc_guard(uint32_t *guard, int flag) {
   ...
   int loc = flag >> 4; // only use when real_flag is 1 or 2
   int real_flag = flag & 0xf;
   if(real_flag == 1)
   {
     start += 1;
     addPath(index);
   ...
   else if(real_flag == 2)
   {
       ...
       --start;
       if (start == 0) {
         // hash and update bitmap
         uint32_t result = 0;
         for(int i=0;i<path_loc;i++)
         {
             computeAndUpdateHashInternal(result, path[i]);
         }
         uint32_t ifwrite = (__shmem_list[loc]->edges[result / 8] >> (result % 8)) & 0x01;
         __shmem_list[loc+phase_num]->edges[result / 8] |= 1 << (result % 8);
         if(ifwrite == 0)
         {
             char record[1024];
             for(int k=0;k<path_loc;k++)
             {   
                 if(k==0)
                     sprintf(record,"%u::%d", result, path[k]);
                 else
                     sprintf(record, "%s-%d", record, path[k]);
             }
             fprintf(fp[loc],"%s\n",record);
             __shmem_list[loc]->edges[result / 8] |= 1 << (result % 8);
             fflush(fp[loc]);
         }
         clearPath();
     }
 ...
 EncodedJSValue JSC_HOST_CALL functionFuzzilli(JSGlobalObject* globalObject, CallFrame* callFrame)
 {
     ...
     if (operation == "FUZZILLI_CRASH"_s) {
         ...
         RETURN_IF_EXCEPTION(scope, { });
         auto cString = cStringFromViewWithString(globalObject, scope, viewWithString);
         RETURN_IF_EXCEPTION(scope, { });
         fprintf(fzliout, "%s\n", cString.data());
         fflush(fzliout);
     }
